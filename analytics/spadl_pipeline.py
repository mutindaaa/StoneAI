...
from analytics.config import (
    PITCH_LENGTH_M as _PITCH_LEN,
)
from analytics.config import (
    PITCH_WIDTH_M as _PITCH_W,
)
from analytics.config import (
    XT_CONVERGENCE_EPS,
    XT_ITERATIONS,
...
def build_training_corpus(
    ...
    for match_id in match_ids:
        try:
            ...
            parts.append(df[["start_x", "start_y", "end_x", "end_y", "type_name", "result_name"]])
            loaded += 1
...
def fit_xt_grid(
    ...
    iterations: int = XT_ITERATIONS,
) -> np.ndarray:
    """
    Fit a w×n_cols xT grid from a corpus of actions using Karun Singh's algorithm.

    Args:
        corpus_df:  DataFrame with start_x/y, end_x/y, type_name, result_name
        n_cols:     Grid columns (pitch length direction)  default 16
        w:          Grid rows   (pitch width direction)    default 12
        pitch_len:  Pitch length in metres
        pitch_w:    Pitch width in metres
        iterations: Max iterations for the value-iteration solve

    Returns:
        np.ndarray of shape (w, n_cols) with xT values per zone
    """

    # --- Map coordinates to grid cell indices (vectorized) ---
    def _cell_col(x):
        return np.clip((x / pitch_len * n_cols).astype(int), 0, n_cols - 1)
    ...
    n_actions = np.zeros((w, n_cols), dtype=float)
    n_shots = np.zeros((w, n_cols), dtype=float)
    n_goals = np.zeros((w, n_cols), dtype=float)
    n_moves = np.zeros((w, n_cols), dtype=float)
    move_mat = np.zeros((w * n_cols, w * n_cols), dtype=float)  # transition counts
    ...
    s_flat = sr[move_mask] * n_cols + sc[move_mask]
    e_flat = er[move_mask] * n_cols + ec[move_mask]
...
def compute_xt(
    ...
    Args:
        ...
        grid:        Optional pre-loaded (w, n_cols) xT grid. If None, the
                     default cached/fitted grid is used.
        n_cols:      Grid columns (default 16)
    ...
    def _col_idx(x):
        return np.clip((x / _PITCH_LEN * n_cols).astype(int), 0, n_cols - 1)
    ...
    df["xt_value"] = np.round(xt_raw, XT_ROUND_DECIMALS)

    n_nonzero = int((df["xt_value"] > 0).sum())
    n_moves = int(move_mask.sum())
    logger.info(
        "xT scored: %d/%d move actions have positive xT  " "(min=%.5f  max=%.5f  mean=%.5f)",
        n_nonzero,
        n_moves,
